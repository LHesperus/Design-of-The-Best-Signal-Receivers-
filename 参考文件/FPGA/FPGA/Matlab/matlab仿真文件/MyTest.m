...
rBandWidth = 40e6;%信号带宽40MHz   ////modify by guang
rTimeWidth = 20e-6;%发射信号时宽
rPRF = 4e3;%雷达发射脉冲重复周期
rPRT = 1 / rPRF;%雷达发射脉冲重复周期（最大不模糊距离：250*300/2 = 37500m，最大不模糊速度：-20m/s~+20m/s)

K = rBandWidth / rTimeWidth;
%ADC参数
Fs = 120e6;%采样频率160MHz           ////modify by guang
Ts = 1 / Fs;
...
%%%频谱显示
% %-------------------------------------频谱计算-------------------------------------
% figure('name','数字下变频频谱显示');
% wavefft_q=fftshift(fft(y_q));wavefft_i=fftshift(fft(y_i));
% showfft_i=abs(wavefft_i);showfft_q=abs(wavefft_q);
% gq=(-Fs/2+Fs/Ntotal:Fs/Ntotal:Fs/2)/(1e6);
% subplot(211);
% plot(gq,20*log10(showfft_q/max(showfft_q)));
% title('数字下变频Q路信号频谱');xlabel('频率(MHZ)'); ylabel('幅度归一化(dB)');
% subplot(212);
% plot(gq,20*log10(showfft_q/max(showfft_q)));
% title('数字下变频I路信号频谱');xlabel('频率(MHZ)'); ylabel('幅度归一化(dB)');
% zoom xon; grid on; 
% axis([min(gq) max(gq) -60 5]);
