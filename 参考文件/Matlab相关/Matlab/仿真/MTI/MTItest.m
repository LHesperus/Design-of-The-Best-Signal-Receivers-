...
%      Chirp(i+fix(number/2)+1)=exp(1j*(pi*(BandWidth/TimeWidth)*(i/Fs)^2));%exp(j*fi)*，产生复数矩阵Chirp
 ...
 figure(1);                             %脉压系数的实部
subplot(3,1,1); plot(real(Chirp));axis([0 300 -1.5 1.5]);title('脉压系数实部');
subplot(3,1,2); plot(imag(Chirp));axis([0 300 -1.5 1.5]);title('脉压系数虚部');
freq=linspace(0,Fs,SampleNumber);
subplot(3,1,3);plot(freq,(abs(fft(Chirp,SampleNumber))));
...
freq=linspace(0,Fs,SampleNumber);
plot(freq,(abs(fft(Echo,SampleNumber))));
% plot(abs(fft(Echo)));
xlabel('Frequency in MHz');
title('线性调频信号频谱');
grid on;
