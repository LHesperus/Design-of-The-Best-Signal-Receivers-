...
plot(abs(IQ_rec));title('send signal envelope')
...
%% ECHO
R_max=(Prf-T)*c/2;
R=1000;   %this code need t_echo<Prf,so R must < R_max here
%R=0;
...
%% Gauss Channel
SNR=0;                        % The algorithm is effective when SNR greater than -10dB
IQ_rec=awgn(IQ_rec,SNR,'measured');
I_rec=real(IQ_rec);
Q_rec=imag(IQ_rec);
...
suptitle('echo')
figure
plot(t,abs(IQ_rec));title('ECHO envelope')
%% lowpass filter
Hd = lowpass63;
h=Hd.Numerator;
hA=h(1:4:end);
...
figure
plot(abs(MTI_OUT(1,:)));title('MTI_{OUT}')
figure
subplot(2,1,1)
plot(real(MTI_OUT))
subplot(2,1,2)
...
%SOCA-CFAR
%alpha=0;beta=1;
protect_len=4;                           %length of protect unit   
...
subplot(3,1,2)
plot(CAFR);title('CA-CAFR threshold,protect_len=4 ,ref_len=8,alpha=0.5,beta=0.5,');
subplot(3,1,3)
Judge_result=MTD-cfar_ratio*CAFR;
plot(Judge_result);title('Judgment result')

%% Calculating distance
p_0=74; %the position of Judge_result when R=0
p_max=find(Judge_result==max(Judge_result));
R_est=(p_max(1)-p_0)/(fs/D)*c/2
